...
`;
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args) {
        const { vel, pressure } = (args[0] || {}) as { vel?; pressure? };
      ...
      dispose() {
        try {
          ...
          if (Common.renderer) {
            ...
            // Dispose all FBOs and textures
            if (this.output?.simulation) {
              for (const key in this.output.simulation.fbos) {
                this.output.simulation.fbos[key]?.dispose();
              }
            }
            if (this.output?.output.material) {
              (this.output.output.material as THREE.Material).dispose();
              this.output.output.geometry?.dispose();
            }
